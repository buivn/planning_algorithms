...
# Description: In this buffer storage, it combines with priority experience replay technique. 
...
# This buffer class: Buffer_v0 consists of state (observation), actions (rotation and linear), 
# rewards, advantages, returns, values, log probabilities
class Buffer_v0:  # stored as ( state, action, reward, next_state ) in SumTree
    # Buffer for storing trajectories
    def __init__(self, state_dims, n_outputs, max_size, gamma=0.99, lam=0.95):
      # A tree for data priority
      self.priority_tree = SumTree(max_size)
      self.observations = np.zeros(
          (max_size, state_dims), dtype=np.float32)
      # # print(self.observation_buffer.shape)
      self.actions = np.zeros((max_size, n_outputs), dtype=np.int32)
      self.advantages = np.zeros(max_size, dtype=np.float32)
      self.rewards = np.zeros(max_size, dtype=np.float32)
      self.returns = np.zeros(max_size, dtype=np.float32)
      self.values = np.zeros(max_size, dtype=np.float32)
      self.log_probs = np.zeros((max_size, n_outputs), dtype=np.float32)
      self.pointer = 0
      self.trajectory_start_index = 0
      self.gamma, self.lamb = gamma, lam
      self.max_size = max_size
    ...
    def get_sample(self, batch_size):
      ...
      for i in range(batch_size):
        ...
        # print("How many samples are taken: ", i)
...
# This buffer class: Buffer_v1 consists of state (observation - laser data, distance to goal, 
# and agent's velocities), actions (rotation and linear), rewards, advantages, returns,
# values, log probabilities
class Buffer_v1:  # stored as ( state, action, reward, next_state ) in SumTree
    # Buffer for storing trajectories
    def __init__(self, state_dims, action_dims, max_size, gamma=0.99, lam=0.95):
      # A tree for data priority
      self.priority_tree = SumTree(max_size)
      self.laser_data = np.zeros(
          (max_size, state_dims), dtype=np.float32)
      self.distance_to_goal = np.zeros(
          (max_size, 2), dtype=np.float32)
    ...
    # Each new experience will have a score of max_prority.
    def store_experience(self, laser_data, distances, velocities, action, reward, value, log_prob):
      # Find the max priority in all the leaf node
      max_priority = np.max(self.priority_tree.tree[-self.priority_tree.capacity:])
      # If max_priority = 0, set it a nonzero value (have chance to be picked)
      if max_priority == 0:
      ...
      self.priority_tree.add(self.pointer, max_priority)   # set the max priority for new experience
      # save the data
      self.laser_data[self.pointer] = laser_data[0]
      self.distance_to_goal[self.pointer] = distances[0]
      self.velocities[self.pointer] = velocities[0]
      self.actions[self.pointer] = action
      self.rewards[self.pointer] = reward
      self.values[self.pointer] = value
    ...
    def get_sample(self, batch_size):
      ...
      if self.pointer > (self.max_size - 1200):
        self.trajectory_start_index = 0
        self.pointer = 0
      return leaf_indices, self.laser_data[i_list], self.distance_to_goal[i_list], \
              self.velocities[i_list], self.actions[i_list], self.advantages[i_list], \
              self.returns[i_list], self.log_probs[i_list]
